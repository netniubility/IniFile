...
public readonly struct PropertyValue : IEquatable<PropertyValue>
{
	...
	{
		this.value = value;
		type = this.value is not null ? this.value.GetType() : typeof(string);
	...
	private static T ConvertTo<T>(PropertyValue pvalue, Func<string, ConversionResult<T>> converter)
		where T : struct
	{
		if (pvalue.value is T typedValue)
		{
			return typedValue;
		}
	...
	{
		if (pvalue.value is bool boolValue)
		{
			return boolValue;
		}
...
#pragma warning restore S3877 // Exceptions should not be thrown from unexpected methods
	...
	{
		return -331038658 + EqualityComparer<Type>.Default.GetHashCode(type);
	}
